...
namespace NodeCasperParser.Cryptography.CasperNetwork
{
    ...
    {
        public static byte[] DecodeBase16(string base16String)
        {
            if (string.IsNullOrEmpty(base16String))
                throw new ArgumentException("The base16 string cannot be null or empty.");

            if (base16String.Length % 2 != 0)
                throw new ArgumentException("The base16 string must have an even length to be valid.");

            byte[] bytes = new byte[base16String.Length / 2];
            for (int i = 0; i < bytes.Length; i++)
            {
                bytes[i] = Convert.ToByte(base16String.Substring(i * 2, 2), 16);
            }
            return bytes;
        }
